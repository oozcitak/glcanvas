...
namespace GLView {
	...
	void GLCanvas2D::OnPaint(System::Windows::Forms::PaintEventArgs^ e) 
	{
		...
		GLView::GLGraphics2D ^ graphics = gcnew GLView::GLGraphics2D(this, e->Graphics);
		...
		glLoadIdentity();
		OnRender(gcnew GLView::Canvas2DRenderEventArgs(graphics));
	...
	System::Void GLCanvas2D::ControlMouseUp(System::Object^ sender, System::Windows::Forms::MouseEventArgs^ e)
	{
		...
		else if (mSelecting)
		{
			...
			if (((Math::Abs(mSelPt1.X - mSelPt2.X) < 2) && (Math::Abs(mSelPt1.Y - mSelPt2.Y) < 2)) || (SelectionMode == SelectMode::PointPick))
				OnMouseSelect(gcnew GLView::Canvas2DMouseSelectEventArgs(mSelPt2, mSelPt2, e->Button));
			else
				OnMouseSelect(gcnew GLView::Canvas2DMouseSelectEventArgs(mSelPt1, mSelPt2, e->Button));
		}
